...
def generate_video_path(video_path, suffix="_captioned"):
    ...
    # Get the directory of the input video to create output in the same location
    video_dir = os.path.dirname(video_path)
    file_name = os.path.basename(video_path)
    name_without_ext = os.path.splitext(file_name)[0]
    extension = os.path.splitext(file_name)[1]
    return os.path.join(video_dir, f"{name_without_ext}{suffix}{extension}")
...
def add_captions_to_video(video_path, timestamps_file, output_path=None, style_name=None):
    ...
    # IMPORTANT: Preserve audio from the original video
    if video.audio is not None:
        final_video = final_video.set_audio(video.audio)
        print("🔊 Audio preserved from original video")
    else:
        print("⚠️ Warning: Original video has no audio track")
